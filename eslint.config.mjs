...
const allPackages = ['packages/simply-gitlab', 'packages/simply-gitlab-core', 'packages/simply-gitlab-mcp'];
...
const corePackage = ['packages/simply-gitlab-core'];
...
const headerRule = [
  ...
  [
    '',
    {
      pattern: ' \\* Copyright \\(c\\) \\d{4}, SimplySF\\.',
      template: ' * Copyright (c) 2026, SimplySF.',
    },
    ' *',
    ' * Licensed under the Apache License, Version 2.0 (the "License");',
    ' * you may not use this file except in compliance with the License.',
...
export default [
  ...
  })),
  {
    ...
    rules: {
      ...
      // Our own identifiers stay camelCase; object properties do not, because GitLab's REST API is
      // snake_case throughout (source_branch, commit_message, per_page, x-next-page). Renaming its
      // fields on the way in and back out again would mean two spellings for every attribute and a
      // translation layer to keep in step, so request and response objects are written the way the
      // wire writes them.
      ...
      'no-restricted-imports': [
        'error',
        {
          patterns: [
            ...
            {
              group: ['@oclif/*', 'oclif'],
              message: 'simply-gitlab-core is a library: no oclif here. Command concerns belong in the CLI package.',
            },
            {
              group: ['node:child_process', 'child_process'],
              message:
                'simply-gitlab-core never spawns a process. Browser launching and CLI wrapping belong in the consumers.',
            },
          ],
        },
      ],
  ...
  ...scoped(allPackages, { env: { mocha: true }, rules: testOverrideRules }, toTestFiles(allPackages)),
];
