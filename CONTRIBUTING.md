...
The [docs site](https://simplysf.github.io/simply-gitlab/) lives in [`site/`](site) — an [Astro Starlight](https://starlight.astro.build/) site, deployed to GitHub Pages by `.github/workflows/docs.yml` on every push to `main` that touches `site/**` or any package's `README.md` or `package.json`.

```sh
pnpm --filter site run dev     # local preview at http://localhost:4321/simply-gitlab/
pnpm --filter site run build   # production build to site/dist, run before opening a PR that touches site/
```

Every page under `site/src/content/docs/reference/` is generated by `site/scripts/sync-command-reference.mjs` from the oclif command block in `packages/simply-gitlab/README.md` — one page per command topic, grouped by the `GROUPS` table at the top of that script. The directory is gitignored and the script runs automatically before `astro dev`/`astro build`, so nothing under `reference/` is hand-edited or committed; if a command's page is wrong, fix the command's summary/description/examples in `src/`, run `pnpm run readme` in the package, and the site follows. A new command topic (a new `simply gitlab <noun>` subtree) needs a new `GROUPS` entry, placed before any entry whose prefix it extends. The script warns about any command no entry matched, so watch its output.

Everything else under `site/src/content/docs/` is hand-authored: the landing page, Get Started, and the guides (credentials, write safety, scripting, MCP server). Keep a guide in sync with the behavior it describes when you change that behavior — a guide that no longer matches `--help` is worse than no guide.
